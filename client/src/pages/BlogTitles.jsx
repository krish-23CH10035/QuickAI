import { HardDriveUpload, Hash, Sparkles } from 'lucide-react'
...
const BlogTitles = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true)
      const prompt = `Generate a blog titles for the keyword ${input} in the category of ${selectedCategories}`
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-[90%]'>
          {blogcategories.map((item, index) => (
            <span onClick={() => setSelectedCategories(item)}
              className={`px-4 py-2 text-sm border rounded-full cursor-pointer ${selectedCategories === item? 'bg-purple-50 text-purple-700' : 
